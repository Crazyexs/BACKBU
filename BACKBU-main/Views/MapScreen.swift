...
struct MapScreen: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            Map(position: .constant(.userLocation(followsHeading: false))) {
                if let t = state.recorder.activeTrip {
                    ...
                    if coords.count > 1 { MapPolyline(coordinates: coords, strokeStyle: StrokeStyle(lineWidth: 6, lineCap: .round)).stroke(.blue) }
                }
                UserAnnotation()
            }
        ...
        .navigationTitle("Map")
        .toolbar { ToolbarItem(placement: .topBarTrailing) { Button { state.location.requestPermissions() } label: { Image(systemName:"location") } } }
